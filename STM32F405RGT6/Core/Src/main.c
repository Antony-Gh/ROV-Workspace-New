...
int main(void) {
  ...
  /* Initialize all hardware BEFORE starting the RTOS kernel */
  /* osKernelStart() never returns, so any code after it is unreachable */
  ...
  // HAL_TIM_Base_Start_IT(&htim6);  // Uncomment if TIM6 is configured
  ...
  /* Code below this point is unreachable - osKernelStart() never returns */
  /* Main loop processing is now handled by RTOS tasks */

  while (1) {
    /* This loop is never entered when using FreeRTOS */
    /* All processing should be done in RTOS tasks */
  }
  ...
  while (1)
  {
	  /* if(pressure_update_flag)
	 	  ...
	 	  if (HAL_GetTick() - last_send_time >= 100)
	 	  {
	 		  ...
	 		  //UpdateVfrHud(0, 0, depth);
	 		  //UpdateAttitude(roll, pitch, depth);
	 		  MavlinkTx_Process();
	 	  }*/
