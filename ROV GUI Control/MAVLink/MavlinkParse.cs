...
public partial class MAVLink
{
    ...
    public class MavlinkParse
    {
        ...
        private byte expectedPayloadLength = 0;
        private bool parsing = false;
        private bool parsingV2 = false;
        ...
        public MAVLinkMessage ReadByte(byte b)
        {
            if (!parsing)
            {
                if (b == MAVLINK_STX || b == MAVLINK_STX_MAVLINK1)
                {
                    parseBuffer.Clear();
                    parseBuffer.Add(b);
                    parseIndex = 1;
                    parsing = true;
                    parsingV2 = (b == MAVLINK_STX);
                }
                return null;
            }
            ...
            // Ensure the buffer size does not exceed the maximum length of a MAVLink packet
            if (parseBuffer.Count > MAVLINK_MAX_PACKET_LEN)
            {
                // Reset if buffer size exceeds max packet size
                parseBuffer.Clear();
                parsing = false;
                return null;
            }

            if (parseIndex == 2) // Payload length
            {
                expectedPayloadLength = b;
                return null;
            }

            // Check if we've reached the expected payload length and process the message
            if ((parsingV2 && parseIndex >= (expectedPayloadLength + MAVLINK_NUM_NON_PAYLOAD_BYTES)) ||
                (!parsingV2 && parseIndex >= (expectedPayloadLength + 8)))
            {
                try
                {
                    // Convert parseBuffer to array and check length
                    var buffer = parseBuffer.ToArray();
                    if (buffer.Length < 2)
                    {
                        // Handle error if the buffer length is less than expected
                        parsing = false;
                        parseBuffer.Clear();
                        return null;
                    }

                    ushort receivedCrc = BitConverter.ToUInt16(buffer, buffer.Length - 2);
                    ushort computedCrc = MavlinkCRC.crc_calculate(buffer, buffer.Length - 2);
                    byte msgid = parsingV2 ? buffer[7] : buffer[5];

                    var info = MAVLINK_MESSAGE_INFOS.GetMessageInfo(msgid);
                    if (info.name != "")
                        computedCrc = MavlinkCRC.crc_accumulate(info.crc, computedCrc);
                    ...
                    var msg = new MAVLinkMessage(buffer, packetTime);
                ...
                catch (Exception ex)
                {
                    ...
                    Console.WriteLine("Error while parsing MAVLink message: " + ex.Message);
                    parsing = false;
                    parseBuffer.Clear();
                    return null;
                }
        ...
        public MAVLinkMessage ReadPacket(Stream BaseStream)
        {
            ...
            if (hasTimestamp)
            {
                ...
                DateTime date1 = new(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
            ...
            MAVLinkMessage message = new(buffer, packettime);
        ...
        public byte[] GenerateMAVLinkPacket20(MAVLINK_MSG_ID messageType, object indata, bool sign = false, byte sysid = 255, byte compid = (byte)MAV_COMPONENT.MAV_COMP_ID_MISSIONPLANNER, int sequence = -1)
        {
            ...
            if (sign)
            {
                ...
                */
                ...
                using (SHA256CryptoServiceProvider signit = new())
                {
                    MemoryStream ms = new();
